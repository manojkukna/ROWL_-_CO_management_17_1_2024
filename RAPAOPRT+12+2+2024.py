...
dataframes = []
csv_files = manoj
name ={"mahesh": mahesh,"manoj": manoj, "rohit":rohit, "piush": piush,"rajesh":rajesh}  #"rajesh":rajesh
...
def metric_label(risk_dict):
    ...
    with total8:
        st.metric(label="NET PROFIT", value=int(risk_dict["Price"].sum()))
    with total9:
        st.metric(label="NET PROFIT", value=int(risk_dict["Price"].sum() / risk_dict["Pw"].sum()))
    with total10:
        st.metric(label="%NET PROFIT%", value=int(risk_dict['Labor Cost'].sum()))
    ...
    with total01:
        st.metric(label='MARKET PRICE ', value=int(risk_dict["Price"].sum()))
    with total02:
        st.metric(label="PAR CARET PRICE", value=int(risk_dict["Price"].sum() / risk_dict["Pw"].sum()) )
    with total03:
        st.metric(label='Labor Cost', value=int(risk_dict['Labor Cost'].sum()))
    ...
    with total2:
        st.metric(label="RoughWeight", value=round(risk_dict["RoughWeight"].drop_duplicates().sum(),2))
    with total3:
        st.metric(label="polish Weight", value=risk_dict["Pw"].sum())
    with total4:
        st.metric(label="parsent", value=f'{risk_dict["Pw"].sum() /risk_dict["RoughWeight"].drop_duplicates().sum() * 100 :,.0f} %')
    ...
    with total7:
        st.metric(label='size', value= f'{float(len(risk_dict)) / risk_dict["RoughWeight"].drop_duplicates().sum():,.0f}')
...
def run(name):
        ...
        if app == "Home":
            ...
            # print( mahesh)
            df =[ manoj,mahesh,  piush , rohit, rajesh , rohit]

            # df =   [manoj ]
            # dataframes.append(df)
            combined_df = pd.concat(df, ignore_index=True)
            combined_df = pd.DataFrame(combined_df)
            combined_df = pd.DataFrame(combined_df.drop_duplicates(keep='last'))
            print( len(combined_df.index) , combined_df.index)
            print(len(combined_df.columns), combined_df.columns)
            print(len(combined_df.index), combined_df)

            breakpoint()

            all_qttar_dict= raf_pidishan(df3= combined_df,Discount=50,Labor_Cost=80)

            print( all_qttar_dict)
            # breakpoint()
...
# streamlit run RAPAOPRT+9+2+2024.py
